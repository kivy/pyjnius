...
if platform == 'android':
    ...
    if jre_home is None:
        if platform == 'win32':
            java64_dir = r"C:\Program Files\Java"
            java_dir = r"C:\Program Files(x86)\Java"
            if exists(java64_dir):
                for i in listdir(java64_dir):
                    if 'jre-' in i:
                        jre_home = java64_dir + "\\" + i
            elif not exists(java64_dir) and exists(java_dir):
                for i in listdir(java_dir):
                    if 'jre-' in i:
                        jre_home = java_dir + "\\" + i
            else:
    ...
    if not jre_home or not exists(jre_home):
        raise Exception('Unable to determine JRE_HOME. Please update Java to the latest version.')
    ...
    if machine() in machine2cpu.values():
        cpu = machine()
    else:
        print(
            """WARNING: Not able to assign machine() = {machine} to a cpu value!
            Using cpu = 'i386' instead!"""
            .format(machine=str(machine()))
        )
        cpu = 'i386'
